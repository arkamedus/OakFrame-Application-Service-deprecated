...
exports.alphaNumericSort = alphaNumericSort;
function capitalizeFirstLetter(string) {
    return string.charAt(0).toUpperCase() + string.slice(1);
}
exports.capitalizeFirstLetter = capitalizeFirstLetter;
function generatePagination(page, page_size, total_items, url_base) {
    var pages = ((total_items / page_size) | 0) + 1;
    var page_links = "";
    for (var i = Math.max(page - 2, 1); i <= Math.min(page + 2, pages); i++) {
        page_links += " <a href=\"" + url_base + "/page/" + i + "\" style=\"padding: 0 5px;cursor:pointer;\"><strong " + (page === i ? 'disabled' : '') + ">" + i + "</strong></a> ";
    }
    var prev = "<button " + (page <= 1 ? 'disabled' : '') + " rel=\"prev\">Previous</button>";
    var next = "<button " + (page >= pages ? 'disabled' : '') + " rel=\"next\">Next</button>";
    if (page > 1) {
...
function listToString(list) {
    ...
    for (var char = 0; char < list.length; char++) {
        string += list[char];
        if (char < list.length - 2) {
            string += ', ';
        }
        else if (char === list.length - 2) {
            string += ' and ';
        }
    }
    return string;
